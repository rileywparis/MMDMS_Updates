...
-- INDEXING
-- 1. Fast seek for per-account ordered reads
IF NOT EXISTS (
    SELECT 1 FROM sys.indexes 
    WHERE name = 'IX_Usages_Utility_Flag_Acct_Date'
      AND object_id = OBJECT_ID('Usages')
)
BEGIN
    CREATE INDEX IX_Usages_Utility_Flag_Acct_Date
    ON Usages (UTILITY, FLAG, ACCT_NO, CUR_DATE);
END
GO

-- 2. Helps the join for LastPhysical
IF NOT EXISTS (
    SELECT 1 FROM sys.indexes 
    WHERE name = 'IX_Usages_Acct_Date'
      AND object_id = OBJECT_ID('Usages')
)
BEGIN
    CREATE INDEX IX_Usages_Acct_Date
    ON Usages (ACCT_NO, CUR_DATE);
END
GO

-- 3. Helps estimation streak scan
IF NOT EXISTS (
    SELECT 1 FROM sys.indexes 
    WHERE name = 'IX_Usages_Acct_Date_Flag'
      AND object_id = OBJECT_ID('Usages')
)
BEGIN
    CREATE INDEX IX_Usages_Acct_Date_Flag
    ON Usages (ACCT_NO, CUR_DATE, FLAG);
END
GO

-- 4. Helps avoid table scans on REF_NO filtering
IF NOT EXISTS (
    SELECT 1 FROM sys.indexes 
    WHERE name = 'IX_Usages_Utility_RefNo'
      AND object_id = OBJECT_ID('Usages')
)
BEGIN
    CREATE INDEX IX_Usages_Utility_RefNo
    ON Usages (UTILITY, REF_NO);
END
GO
...
--ALTER ESTIMATION PROCEDURE
/****** Object:  StoredProcedure [dbo].[usp_GetEstimatedUsage]    Script Date: 11/18/2025 9:28:44 PM ******/
SET ANSI_NULLS ON
GO
