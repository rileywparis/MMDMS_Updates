...
CREATE TABLE StaticFlags (
    Flag VARCHAR(50) PRIMARY KEY
);

INSERT INTO StaticFlags (Flag)
VALUES ('CHANGEOUT'), ('ROLLOVER'), ('NEW UTILITY');
...
UPDATE Usages
SET FLAG = REF_NO
WHERE REF_NO IN ('CHANGEOUT','ROLLOVER','NEW UTILITY');
...
BEGIN
    ...
    ;WITH OrderedPhysical AS (
        ...
        WHERE UTILITY = @Utility
          AND FLAG = 'PHYSICAL'
          AND REF_NO NOT IN (SELECT Flag FROM StaticFlags) --match DbCfg StaticFlags
    )
    SELECT
        p1.ACCT_NO,
